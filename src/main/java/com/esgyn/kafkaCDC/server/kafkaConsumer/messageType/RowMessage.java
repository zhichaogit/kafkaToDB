...
public class RowMessage<T> {
    ...
    public boolean init(MessageTypePara<T> mtpara_) throws UnsupportedEncodingException {
        ...
        if (log.isTraceEnabled()){
            log.trace("enter function [schema: " + schemaName + ", table: "
                  + tableName + ", delimiter: \"" + delimiter_ 
                  + "\", thread id: " + mtpara.getThread() + ", message [" + mtpara.getMessage() 
                  + "]");
        }
    ...
    public Boolean AnalyzeMessage() {
        ...
        try {
            message = new String(((String) message_).getBytes(mtpara.getEncoding()), "UTF-8");
                ...
                log.error ("the encoding is not supported in java [" 
                        + usee.getMessage() + "]");
